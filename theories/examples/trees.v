...
Section fin_list_sum.
  ...
  Proof.
    ...
    + finite union; [ finite dec left | ].
      finite compose.
      intros (a,b); simpl; intros (? & ? & ?).
      finite compose.
      apply fin_product with (P := λ l, list_sum f l = a)
                             (Q := λ l, list_sum f l = b).
      all: apply IHj; lia.
  Qed.
...
Proof.
  ...
  apply fin_equiv with (P := λ t, ∃l, ⟨l⟩ᵣ = t ∧ 1+list_sum (λ t, ⌊t⌋ᵣ) l = n).
  + intros t; split.
    * intros (? & <- & ?); rewrite rtree_size_fix; auto.
    * destruct t as [l]; intros <-; exists l; rewrite rtree_size_fix; auto.
  + finite compose.
    destruct n as [ | n ].
    1: finite as (λ _, False); lia.
    finite as (λ l, list_sum (λ t, ⌊t⌋ᵣ) l = n).
    1: intro; lia.
...
Section fin_ltree_weight.
  ...
  Proof.
    ...
    apply fin_equiv
      with (P := λ t, ∃k, (∃l, (∃x, ⟨x|l⟩ₗ = t ∧ w x = S k)
